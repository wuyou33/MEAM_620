clear;
clc;
map = load_map('map1.txt', 0.1, 2, 0.3);
% start = [1.81 2.40 5.5];
% goal = [9.8 8.7 0.2];
% start = [1.1 2.1 3.1];
% goal = [9.9 8.9 5.9];
...
%             new_reachable_group(reachable_index_min(2):reachable_index_max(2), reachable_index_min(1):reachable_index_max(1), reachable_index_min(3):reachable_index_max(3)) = 1;
            ...
            r1_new = r1_new & empty;
            new_reachable_not_visited = r1_new & ~v1;
            new_reachable = new_reachable_not_visited & (r1 ~= 1);
            old_reachable = (r1 == 1) & new_reachable_not_visited;
            ...
            r2_new(reachable_index_min(1):reachable_index_max(1), reachable_index_min(2):reachable_index_max(2), reachable_index_min(3):reachable_index_max(3)) = ...
                cost(reachable_index_min(1) - x_idx + 2:reachable_index_max(1) - x_idx + 2, reachable_index_min(2) - y_idx + 2:reachable_index_max(2) - y_idx + 2,...
                reachable_index_min(3) - z_idx + 2:reachable_index_max(3) - z_idx + 2); % cost of the reachable
            
            
            
            
            r1(new_r_idx) = 1;
            r2(new_r_idx) = v2(idx) + r2_new(new_r_idx);
            
            temp = inf(size(r2));
            temp(old_r_idx) = v2(idx) + r2_new(old_r_idx);
            R2 = bsxfun(@min, r2, temp);
            
            % SET PARENTS
%             old_r_p = find(bsxfun(@lt, r2_new, r2) == 1);
%             old_r_p = find((r2 ~= R2) == 1);
            r2 = R2;
