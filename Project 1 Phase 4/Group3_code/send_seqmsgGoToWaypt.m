...
for c=1:numquads
    ...
    %just sit and idle
    seq(end).type = 902; %this is the TYPE that determines what happens in the low level code
    seq(end).time = t_inf;%this tells the quad to hover until another message is received
    
    %seq(end).pos = waypts(1, :);   % go to waypoint
    seq(end).pos = [-2.0000   -1.0000    1.0000];   % back to landing point
