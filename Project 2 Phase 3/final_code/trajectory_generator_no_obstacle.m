function [pos, phi, phi_d, vel, acc, total_t] = trajectory_generator_no_obstacle(t, varargin)
...
path_vel_max = 1;  % maximum of speed, 20m/s.
...
if t >= end_time
        ...
        end_point = path0(valid_start + 1, :);    % to follow points
        if (1)
            % trajectory from start_point to end_point
            start_time = time(valid_start);
            end_time = time(valid_start + 1);
            ...
            while norm(acc_max(1, 1:2)) >= 0.5 || abs(acc_max(1, 3)) >= 0.5 || norm(acc_max(2, 1:2)) >= 0.5 || abs(acc_max(2, 3)) >= 0.5
